...
> **2026-09-21 — build `20260921b` BUILT (Google sign-in as a same-origin OpenID redirect + visible sign-in errors — DECISIONS 09-21); `20260921a` (Phase 2) is LIVE** (multi-user Phase 2: People screen, invite link → `createInvite`, join page `?j=CODE` → `acceptInvite`, a helper's view of the owner's ReCall with footer by role and *Log item · in X's ReCall*, owner tag on tiles, *Show who added each photo*, remove / leave / expired copy, the switcher, Settings Account card rewritten; adopted places repaired; rules: shared-with-me listener provable, invites readable by code, grantee may leave — REDEPLOY rules + indexes with the push). Rig 93/93 + roles 41/41; real rules engine (`04_Engineering/firebase/rules-test/`) green. **Live state:** `20260919a` + Phase 1 rules (legacy clauses still in); Firebase deployed 2026-09-21 as tangadi.biz@gmail.com; Dad's phone owns every pre-09-19 doc, Ravi's phone is read-only until Dad invites him (the Phase 2 flow is the fix — OPEN_ITEMS). Earlier: Phase 1 (`20260919a`: owner/roles on every doc, merged listeners, legacy adoption, auth upgrade; rules + functions in `04_Engineering/firebase/`). Earlier: build `20260916f` (8b: place picker, tidy-up; round 8: the thing card rebuilt on the things/places/sightings model — `06_Handoffs/design/DESIGN_2026-09-16_things-places-sightings.md`; 7c: fixed action bar, Earlier photos fixed; 7b: Ravi's four fixes — see DECISIONS; both cache stamps now bumped) (round 7, `06_Handoffs/design/BOARD_2026-09-15_round7.md`: mockups → Ravi's rulings → code; when pill + Private on one line; private = per person; pin badge for *no place yet*; *Text size & colours*; Locations with place photos; *Where is it?* in three views; 4 shots per Add photo; **multi-user RESET 2026-09-19** — Margaret owns her things, Drive-style sharing; plan + 16 screens in `06_Handoffs/design/PLAN_2026-09-19_multi-user.md`, Ravi said "proceed" 09-19; Phase 1 built). **Earlier: 2026-09-14 — v0.2 is LIVE (stamp `20260905l`); build `20260914p`** (n: thing card layout A, chosen by Ravi from rendered options)** (m: private items — exclusion model, one switch; rules enforce it in multi-user stage 1)** (l: thing card = Add photo · Edit, no *Found it*; wrong-photo guard; hold on the
