...
- [ ] **Firebase (2026-09-21, done so far):** CLI signed in as **tangadi.biz@gmail.com**, project `recall-d9886`; Google sign-in enabled (public name *ReCall*); indexes deployed; five functions deployed on **Node 22** (us-central1; `firebase.json` sets the runtime — a runtime-only change needs a source edit + `--force`, the CLI skips "unchanged" code); image cleanup 1 day. **Still to do:** git push `20260919a`; phone → Settings → Account → *Legacy docs left: 0*; THEN `firebase deploy --only firestore:rules`; confirm the grid; then strip the `legacy()` clauses and redeploy rules.
