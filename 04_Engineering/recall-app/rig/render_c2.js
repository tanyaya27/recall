...
(async () => {
  ...
  const p = await ctx.newPage(); fs.mkdirSync('shots/c2', { recursive: true });
  const only = process.argv[2];
  for (const f of fs.readdirSync('mock').filter((f) => f.startsWith('c2_') && f.endsWith('.html') && (!only || f.includes(only))).sort()) {
    await p.goto('file://' + path.resolve('mock', f)); await p.waitForLoadState('load'); await p.waitForTimeout(120);
    await p.screenshot({ path: 'shots/c2/' + f.replace('.html', '.png') });
  }
  await b.close();
})();
