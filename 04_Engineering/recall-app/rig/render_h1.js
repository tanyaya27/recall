...
(async () => {
  ...
  const p = await ctx.newPage(); fs.mkdirSync('shots/h1', { recursive: true });
  const only = process.argv[2];
  for (const f of fs.readdirSync('mock').filter((f) => f.startsWith('h1_') && f.endsWith('.html') && (!only || f.includes(only))).sort()) {
    await p.goto('file://' + path.resolve('mock', f)); await p.waitForLoadState('load'); await p.waitForTimeout(120);
    await p.screenshot({ path: 'shots/h1/' + f.replace('.html', '.png') });
  }
  await b.close();
})();
